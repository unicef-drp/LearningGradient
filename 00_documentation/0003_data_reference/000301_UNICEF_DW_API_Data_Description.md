...
**Purpose**: This document describes the **data returned by the UNICEF Data Warehouse API** for the Learning Gradient dataflow. This repo consumes that payload by calling the CSV endpoint via `readr::read_csv`.
...
- **0201_load.R** loads this structure from the API via `read_csv(paste0(dw_api_url, "?format=csv"))`. Set `dw_api_url` in `project_config.R`.
